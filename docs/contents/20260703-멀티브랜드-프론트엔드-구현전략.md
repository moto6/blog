...
```
                        ┌── VITE_APP_MODE=place ──▶  room(회의실) 번들 ──▶ room.dunhill-industry.com
소스코드 1벌 ──(빌드)──┤                                                      │
                        └── VITE_APP_MODE=hub ────▶  portal(포털) 번들 ────▶ portal.dunhill-industry.com
                                                                              │
                                                        통합백엔드: hub-be.dunhill-industry.com (1개)
```
...
| 제약 | 내용 |
|------|------|
| 도메인 | `place-dev.dunhill-industry.com`(회의실 예약) / `hub-dev.dunhill-industry.com`(크루정보·프로젝트) |
| 백엔드 | `hub-be-dev.dunhill-industry.com` **하나로 통합** — 어차피 사용자에겐 안 보임 |
...
```tsx
export const SAWOO_INFO: NavItem = {
  path: '/sawoo-info',        // 라우트 경로 겸 GNB 링크 (단일 소스)
  label: '사우님들의정보',
  icon: Users,       // 루시드리액트 아이콘 지정
...
const PLACE: AppRecipe = {
  ...
  headerLabel: '공간관리',          // 헤더 "dunhill-industry | ___"
  loginBrand: 'Place',
  loginSubtitle: '회의실 예약 시스템',
  homePath: '/room-reservation',   // ⚠️ 리터럴이어야 함 (§5 함정)
